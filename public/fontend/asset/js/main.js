...
function hendlModalFormAuth() {
  ...
  var fromAuthBtnList = document.querySelectorAll(
    ".modal-auth__form-support-link"
  );
  ...
  function formAuthDisplay(fromAuthBtn) {
    return () => {
      ...
      if (formAuthName) {
        var formAuthSetActive = document.querySelector(
          ".modal-auth__form-" + formAuthName
        );
        if (formAuthActive) {
          formAuthHidden();
        }
...
function hendlSubMenuMobile() {
  this.subMenuIconPlus = document.querySelector(
    ".modal-menu-mobile__list-items-icon-plus"
  );
  this.subMenuIconMinus = document.querySelector(
    ".modal-menu-mobile__list-items-icon-minus"
  );
  this.subMenuCheckbox = document.getElementById(
    "modal-menu-mobile__submenu-checkbox"
  );
  this.listMenu = document.querySelector(".modal-menu-mobile__submenu-list");
  this.subMenuMobileDisplay = () => {
    return () => {
      this.isActive = document.getElementById(
        "modal-menu-mobile__submenu-checkbox"
      ).checked;
...
function hendlSlideShow(nameSlideShow, autoPlay) {
  ...
  this.start = () => {
    if (this.slideShowEle) {
      this.listSlidesEles = this.slideShowEle.querySelectorAll(
        ".slideshow-content__slides"
      );
      this.listDotEles = this.slideShowEle.querySelectorAll(
        ".slideshow__control-dot"
      );
      this.nextEle = this.slideShowEle.querySelector(
        ".slideshow__control-next"
      );
      this.prevEle = this.slideShowEle.querySelector(
      ...
      if (this.listSlidesEles.length > 0) {
        ...
        if (this.listDotEles.length > 0) {
          this.listDotEles[this.currentIndex - 1].classList.add(
            "slideshow__control-dot--active"
          );
        }
      }
      return true;
  ...
  this.next = () => {
    if (this.listSlidesEles) {
      ...
      this.listSlidesEles[this.currentIndex - 1].classList.remove(
        "slideshow-content__slides-in-right",
        "slideshow-content__slides-in-left",
        "slideshow-content__slides-out-right"
      );
      this.listSlidesEles[this.currentIndex - 1].style.left = "100%";
      this.listSlidesEles[this.currentIndex - 1].classList.add(
        "slideshow-content__slides-out-left"
      );

      if (this.listDotEles.length > 0) {
        this.listDotEles[this.currentIndex - 1].classList.remove(
          "slideshow__control-dot--active"
        );
        this.listDotEles[nextIndex - 1].classList.add(
          "slideshow__control-dot--active"
        );
      }

      this.listSlidesEles[nextIndex - 1].classList.remove(
        "slideshow-content__slides-out-right",
        "slideshow-content__slides-in-left",
        "slideshow-content__slides-out-left"
      );
      this.listSlidesEles[nextIndex - 1].style.left = "0";
      this.listSlidesEles[nextIndex - 1].classList.add(
        "slideshow-content__slides-in-right"
      );
      if (this.timer) {
        clearInterval(this.timer);
  ...
  this.prev = () => {
    if (this.listSlidesEles) {
      ...
      if (this.listDotEles.length > 0) {
        ...
        this.listDotEles[prevIndex - 1].classList.add(
          "slideshow__control-dot--active"
        );
      }

      this.listSlidesEles[prevIndex - 1].classList.remove(
        "slideshow-content__slides-out-right",
        "slideshow-content__slides-in-left",
        "slideshow-content__slides-out-left",
        "slideshow-content__slides-in-right"
      ...
      this.listSlidesEles[prevIndex - 1].classList.add(
        "slideshow-content__slides-in-left"
      );
      if (this.timer) {
        clearInterval(this.timer);
        ...
        dot.onclick = (e) => {
          ...
          if (indexSpecified && Number(indexSpecified)) {
            if (parseInt(indexSpecified) > this.currentIndex) {
              ...
              this.listDotEles[parseInt(indexSpecified) - 1].classList.add(
                "slideshow__control-dot--active"
              );
              this.listSlidesEles[
                parseInt(indexSpecified) - 1
              ].classList.remove(
                "slideshow-content__slides-out-right",
                "slideshow-content__slides-in-left",
                "slideshow-content__slides-out-left"
              );
              this.listSlidesEles[parseInt(indexSpecified) - 1].style.left =
                "0";
              this.listSlidesEles[parseInt(indexSpecified) - 1].classList.add(
                "slideshow-content__slides-in-right"
              );
...
function main() {
  ...
  const productDeatilSlide = new hendlSlideShow(
    ".app-product-detail-slide",
    false
  );
